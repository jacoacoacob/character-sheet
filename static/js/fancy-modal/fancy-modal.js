...
function setupFancyModal(appContext) {
    ...
    window.addEventListener("keydown", (ev) => {
        if (isCommandKey(ev) && isLetterKey(ev, "k")) {
            ev.preventDefault();
            appContext.notifications.requestOpen("fancy_modal", { payload: TAB_CAMPAIGN_NOTE, force: true });
        }
        if (isCommandKey(ev) && isLetterKey(ev, "s")) {
            ev.preventDefault();
            appContext.notifications.requestOpen("fancy_modal", { payload: TAB_SAVE_CHANGES, force: true });
        }
    });


    appContext.notifications.onOpen("fancy_modal", ({ status, payload }) => {
        if (status === "success") {
